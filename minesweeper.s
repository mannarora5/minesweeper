...
reveal_grid__body:
        ...
        #       grid[row][col] |= IS_RVLD_MASK;  grid[row][column] is OR w 0x20
        #     }
        #   }
        # }
        ...
        main_row_init:
                li      $t0, 0 # initialise row = 0
        main_row_cond:
                bge     $t0, N_ROWS, main_row_end # if (row >= N_ROWS) goto main_row_end
        main_row_body:
        main_col_init:
                li      $t1, 0 # initialise col = 0
        main_col_cond:
                bge     $t1, N_COLS, main_col_end # if (col >= N_COLS) goto main_col_end
        main_col_body:
                mul     $t2, $t0, N_COLS 
                add     $t2, $t2, $t1 # offset
                lb      $t3, grid($t2) # $t3 = grid[row][column]

                or      $t3, $t3, IS_RVLD_MASK   # $t3 = grid[row][column] | IS_RVLD_MASK
                sb	$t3, grid($t2)		# store byte
        main_col_step:
                addi    $t1, $t1, 1 # col++;
                j	main_col_cond # jump to col condition 
        main_col_end:
        
        main_row_step:
                addi    $t0, $t0, 1 # row++;
                j	main_row_cond # jump to row condition
        main_row_end:
                j       reveal_grid__epilogue # jump to reveal__grid_epilogue
...
place_bombs__body:
        ...
        main_i_init:
                li      $t6, 0	# int i = 0 /  $t6 to avoid getting overriden
		lw	$t7, total_bombs # load data / $t7 to avoid getting overriden
			
        main_i_cond:	
                bge     $t6, $t7, main_i_end # if ( i >= total_bombs) goto main_i_end
        main_i_body:

		move 	$s0, $a0 # store $a0
		move    $s1, $a1 # store $a1
		jal	place_single_bomb   # place_single_bomb($a0, $a1)
		move 	$a0, $s0 # reload $a0
		move    $a1, $s1 # reload $a1

        main_i_step:
	      
		addi	$t6, $t6, 1 #i++;
		j	main_i_cond # jump to cmain_i_cond
        main_i_end:
                j	place_bombs__epilogue # jump toplace_bombs__epilogue
                

place_bombs__epilogue:
        lw      $ra, 0($sp)
	lw      $s0, 4($sp) # memory for s0
        lw 	$s1, 8($sp) # memory for s1
...
mark_cell__body:
	...
	# $a0 = row --> need to save in stack
	# $a1 = col --> need to save in stack
        
	        move	$s2, $a0 # store row
		move    $s3, $a1 # store int
                lw      $s4, bomb_count 
                lw      $s5, debug_mode 
	...
	if_rvld_before:
		...
		mul     $t0, $s2, N_COLS 
                add     $t0, $t0, $s3  # offset
                lb      $t1, grid($t0) # $t1 = grid[row][column]
                move    $s6, $t1 # store in memory
		
        if_rvld:
                and     $t2, $s6, IS_RVLD_MASK # $t2 = grid[row][col] & IS_RVLD_MASK
		beqz    $t2, if_mrkd # if (grid[row][col] & IS_RVLD_MASK)
	        
        if_rvld2:
		beqz    $s5, if_rvld_msg # if(debug_error)
		j       mark_cell__epilogue				    
		
	if_rvld_msg:
		la      $a0, reveal_error # printf("Cannot mark a revealed cell.\n"); 
		li      $v0, 4
		syscall
		j       mark_cell__epilogue
	...
	if_mrkd:
		...
		and     $t2, $s6, IS_MRKD_MASK # $t2 = grid[row][col] & IS_MRKD_MASK
		beqz    $t2, else_mrkd # if(grid[row][col] & IS_MRKD_MASK)
                la      $t3, IS_MRKD_MASK 
                not     $t3, $t3 # $t3 = ~IS_MRKD_MASK
                and     $t4, $s6, $t3 # $t4 = grid[row][col] & ~IS_MRKD_MASK
                sb      $t4, grid($t0)
                addi    $s4, $s4, 1 # bomb_count++:
                sw      $s4, bomb_count
                j       mark_cell__epilogue
	...
	else_mrkd:
		...
		or      $t2, $s6, IS_MRKD_MASK # $t2 = grid[row][col] | IS_MRKD_MASK 
		sb      $t2, grid($t0)
		addi    $s4, $s4, -1 # bomb_count--;
		sw      $s4, bomb_count
...
reveal_cell__prologue:
        addiu   $sp, $sp, -28 # make storage in stack
        sw      $ra, 0($sp)
        sw      $s0, 4($sp)
        sw      $s1, 8($sp)
        sw      $s2, 12($sp)
...
reveal_cell__body:
        ...
        lw      $s4, debug_mode # load debug_mode
        lw      $s5, cells_left # load cells_left

        mul     $t0, $s0, N_COLS 
        add     $t0, $t0, $s1  # offset
        lb      $t1, grid($t0) # $t1 = grid[row][column]
        move    $s2, $t1 # store grid in $s2

        if_cell_bomb:

                and     $s3, $s2, IS_BOMB_MASK # $s3 = grid[row][col] & IS_BOMB_MASK
                beqz    $s3, if_curr_marked # if (grid[row][col] & IS_BOMB_MASK)

                li      $t3, LOSE
                sw      $t3, game_state # game_state = LOSE

        if_curr_marked:

                and    $t2, $s2, IS_MRKD_MASK # $t2 = grid[row][col] & IS_MRKD_MASK
                beqz   $t2, if_rvld_ # if(grid[row][col] & IS_MRKD_MASK)

        if_curr_marked_inner:

                beqz    $s4, if_curr_marked_print # if(debug_mode)
                j       reveal_cell__epilogue

        if_curr_marked_print:
        ...
        if_rvld_:
                ...
                and     $t2, $s2, IS_RVLD_MASK # $t2 = grid[row][col] & IS_RVLD_MASK
                beqz    $t2, if_reveal_cell # if(grid[row][col] & IS_RVLD_MASK)

        if_rvld2_:
        
                beqz    $s4, if_rvld_print # if(debug_mode)
                j       reveal_cell__epilogue
        ...
        if_reveal_cell:
                and     $t2, $s2, VALUE_MASK # $t2 = grid[row][col] & VALUE_MASK
                bnez    $t2, if_reveal_cell_else # if((grid[row][col] & VALUE_MASK) == 0)
                move 	$a0, $s0 # load $a0
		move    $a1, $s1 # load $a1
                  
                jal     clear_surroundings
                
                j       if_reveal_cell_last
        if_reveal_cell_else:
               
                or      $s2, $s2, IS_RVLD_MASK # grid[row][col] = grid[row][col] & IS_RVLD_MASK
                sb      $s2, grid($t0)
                beq     $s3, LOSE, if_reveal_cell_last # if (game_state != LOSE)
                
                addi    $s5, $s5, -1 # cells_left--;
                sw      $s5, cells_left 
                j       if_reveal_cell_last


        if_reveal_cell_last:
                bnez     $s5, reveal_cell__epilogue # if (cells_left == 0)
                li       $t5, WIN
                sw       $t5, game_state  # game_state = WIN
...
clear_surroundings__body:
        ...
        move    $s0, $a0 # store row
        move    $s1, $a1 # store col
 
        mul     $t0, $s0, N_COLS 
        add     $t0, $t0, $s1  # offset
        lb      $t1, grid($t0) # $t1 = grid[row][col]
        move    $s2, $t1 # $s2 = grid[row][col]

        lw      $s3, cells_left # $s3 = cells_left

        if_big_1:
                bltz    $s0, clear_surroundings__epilogue # if (row < 0)
        if_big_2: 
                bge     $s0, N_ROWS, clear_surroundings__epilogue # if (row >= N_ROWS)
        if_big_3:
                bltz    $s1, clear_surroundings__epilogue # if (col < 0)
        if_big_4:
                bge     $s1, N_COLS, clear_surroundings__epilogue # if (col >= N_COLS)
        if_and_rvld_mask:
                and     $t2, $s2, IS_RVLD_MASK # grid[row][col] & IS_RVLD_MASK
                bnez    $t2, clear_surroundings__epilogue # if (grid[row][col] & IS_RVLD_MASK)

        reveal_the_cell:
                or      $t5, $s2, IS_RVLD_MASK # grid[row][col] |= IS_RVLD_MASK;
                sb      $t5, grid($t0)

                lb      $s2, grid($t0)
                addi    $s3, $s3, -1 # cells_left--;
                sw      $s3, cells_left
                

        unmark_cell_if_marked:
                li      $t6, IS_MRKD_MASK
                not     $t6, $t6 # ~IS_MRKD_MASK
                and     $s4, $s2, $t6 # grid[row][col] &= ~IS_MRKD_MASK;
                sb      $s4, grid($t0)
                lb      $s2, grid($t0)

        
        stop_reveal:
                and     $t7, $s2, VALUE_MASK # grid[row][col] & VALUE_MASK
                bne     $t7, 0, clear_surroundings__epilogue # if (grid[row][col] & VALUE_MASK)
                

        recursion: 
        
                arg1: # clear_surroundings(row - 1, col);
                        move    $a0, $s0                
                        addi    $a0, $s0, -1
                        move    $a1, $s1
                        jal     clear_surroundings

                arg2: # clear_surroundings(row - 1, col - 1);
                        addi    $a0, $s0, -1
                        addi    $a1, $s1, -1            
                        jal     clear_surroundings

                arg3: # clear_surroundings(row - 1, col + 1);
                        addi    $a0, $s0, -1
                        addi    $a1, $s1, 1             
                        jal     clear_surroundings

                arg4: # clear_surroundings(row, col - 1);

                        move    $a0, $s0                 
                        addi    $a1, $s1, -1         
                        jal     clear_surroundings      

                arg5: #  clear_surroundings(row, col + 1);

                        move    $a0, $s0
                        addi    $a1, $s1, 1             
                        jal     clear_surroundings

                arg6:  #  clear_surroundings(row + 1, col - 1);

                        addi    $a0, $s0, 1                     
                        addi    $a1, $s1, -1            
                        jal     clear_surroundings

                arg7: #  clear_surroundings(row + 1, col);

                        addi    $a0, $s0, 1                
                        move    $a1, $s1            
                        jal     clear_surroundings

                arg8: #  clear_surroundings(row + 1, col + 1);
...
bombs_error:
        .asciiz         "Bombs must be in the range 1 to 91 inclusive.\n"
